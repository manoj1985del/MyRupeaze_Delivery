...
public class AdapterAwaitingMedicalOrders extends RecyclerView.Adapter<AdapterAwaitingMedicalOrders.ViewHolder> {

    private List<AwaitingOrderListing> m_lstOrders;
    private Context m_Context;

    public AdapterAwaitingMedicalOrders(List<AwaitingOrderListing> lstOrders, Context ctx){
    ...
    @NonNull
    @Override
    public AdapterAwaitingMedicalOrders.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.activity_awaiting_order_listing, parent, false);
        return new AdapterAwaitingMedicalOrders.ViewHolder(v, m_Context);
    }

    @Override
    public void onBindViewHolder(@NonNull AdapterAwaitingMedicalOrders.ViewHolder holder, int position) {

        AwaitingOrderListing listItem = m_lstOrders.get(position);

        PharmacistRequests pharmacistRequests = listItem.getPharmaRequest();

        User user = pharmacistRequests.user;
